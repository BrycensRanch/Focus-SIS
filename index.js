// I am free Every since thing I've done I've decided to do My actions are
// governed by but nothing my free will

const SISFocusBaseURL = 'https://sis.palmbeachschools.org/focus';

let {getEdgePath} = require('edge-paths');
// puppeteer-extra is a drop-in replacement for puppeteer, it augments the
// installed puppeteer with plugin functionality
...
async function runTheSexyCode() {
  ...
  async function LoginToSIS() {
    ...
    if (loginFormExists) 
      throw new Error('Failed to login, RIP!');
    console.log(5);
  }
  ...
  try {
    const googleClassroomCoursesString = await fs.readFile(
      './googleClassroomCourses.json'
    );
  ...
  // await page.waitForNavigation({     waitUntil: 'networkidle2' }); await
  // browser.close()
  const cursor = createCursor(page);
  ...
  // let responseJson = await finalResponse.json(); var bodyFormData = new
  // FormData(); const requestData = { } for ( var key in requestData ) {
  // bodyFormData.append(key, requestData[key]); } const responseJson = await
  // axios("${SISFocusBaseURL}/classes/FocusModule.class.php?force_package=SIS&modname=Grades%2FStudentRCGrades.php&details=false",
  // {     method: "POST",     cookies: cookies,     data: bodyFormData,
  // headers: {         "User-Agent": userAgent,
  // "Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"     } })
  await page.setRequestInterception(true);

  // page.on('request', interceptedRequest => {     if
  // (interceptedRequest.isInterceptResolutionHandled()) return;
  // console.log("INTERCEPTED REQUEST!!",interceptedRequest.url(),
  // interceptedRequest.method(), interceptedRequest.headers());
    ...
    if (request.redirectChain().length === 0) {
      ...
      if (request.url().includes('StudentRCGrades.php')) {
        ...
        try {
          await fs.writeFile(
            './all-grades.json',
            JSON.stringify(responseBody[0].result.rows, null, 2)
          );
        } catch (e) {
          console.error(e);
          return;
        }
        // const activeClassesRows = responseBody[0].result.rows.filter(r =>
        // r.school_name.includes("HIGH") && r.syear === (new Date().getFullYear() - 1)
        // && Number(r.credits_earned) <= 1)
        const activeClassesRows = responseBody[0].result.rows.filter(
          (r) => classes.find((c) => c.courseId === r.course_number)
        );
        const responseJson = responseBody;

        console.log(util.inspect(activeClassesRows, {
          showHidden: false,
          depth     : null
        }));
        ...
        await activeClassesRows.forEach(async (r) => {
          const gradeForClass = {
            ...
            name             : r.course_name,
            room             : r.room,
            courseId         : r.course_number,
            teacher          : r.teacher_name,
            grade            : r[`${quarter.toLowerCase()}_mp_grade`] || 'NG',
            school_name      : r.school_name,
            school_year      : r.syear,
            school_year_human: r.syear_display,
            assignments      : [],
            googleClassroom  : {}
          };
        ...
        for (const activeClass of classes) {
          const detailedClassDetails = responseJson[0].result.rows.find(
            (r) => r.course_number === activeClass.courseId
          );
          ...
          const classAssignments = await rawClassAssignments.map((c) => {
            ...
            return c[2].includes('/')
              ? {
                ...
                lastModified: !!c[8].trim()
                  ? parser.fromString(c[8])
                  : null,
                category    : c[9],
                resources   : c[10]
              }
          ...
          await fs.writeFile(
            `./assignments-${detailedClassDetails.teacher_name} - ${detailedClassDetails.course_name} (${detailedClassDetails.period_name}).json`,
        ...
        const user = {
          fname : firstrow.student_first_name,
          lname : firstrow.student_last_name,
          number: Number(firstrow.student_id)
        };
        console.log(
          `Grades for ${user.fname} ${user.lname} (${user.number}) are:\n${grades.filter((g) => g.school_name.includes('HIGH')).map((r) => JSON.stringify(r)).join('\n')}`
        );
